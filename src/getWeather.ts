export const getWeather = (city: string, unitGroup: string, updateResponse: (stringifiedRes: any) => void) => {
    fetch(`https://weather.visualcrossing.com/VisualCrossingWebServices/rest/services/timeline/${city}?unitGroup=${unitGroup}&key=GTK8Z96XLPV5HJ3KGXK5E9JP7&contentType=json`)
        ...
        .then((objectRes) => updateResponse(objectRes)) // save response data in response object
        .catch((error) => {
